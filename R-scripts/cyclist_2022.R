...
cyclist_2022 <- do.call(rbind, csv_list)

## Check Structure of Cyclist Dataset
str(cyclist_2022)

## Saving Cyclist 2022 file into system 
write.csv(cyclist_2022,"cyclist-2022.csv", row.names = FALSE)  # Since we stored the local file in our system now we can use readr function to load dataset
...
## Load file 
library(readr)
cyclist_2022 <- read_csv("cyclist-2022.csv")
head(cyclist_2022)
...
colSums(is.na(cyclist_2022[columns_to_check]))

## Formatting 'started_at' and 'ended_at' columns
cyclist_2022$started_at <- as.POSIXct(cyclist_2022$started_at, format = "%Y-%m-%d %H:%M:%S")
cyclist_2022$ended_at <- as.POSIXct(cyclist_2022$ended_at, format = "%Y-%m-%d %H:%M:%S")

str(cyclist_2022)

## Extract month name from 'started_at' and add it as a new column
cyclist_2022$month <- month.name[month(cyclist_2022$started_at)]
...
cyclist_2022$Total_time_secs <- as.numeric(cyclist_2022$ended_at - cyclist_2022$started_at)

## Group by rideable type, member/casual status, and month_name, and calculate the mean ride time
Mean_Time_C22 <- cyclist_2022 %>%
...
## Save Mean_Time 
write.csv(Mean_Time_C22,"Mean-ridetime-cyclist2022.csv", row.names = FALSE)


# Step 4: Plot Mean Ride Time Over Months

## Ensure months are ordered chronologically
Mean_Time_C22$month <- factor(Mean_Time_C22$month, 
                              levels = c("January", "February", "March", "April", "May","June", "July", "August", "September", "October", "November", "December"))


## Plot the mean ride time across months
ggplot(Mean_Time_C22, aes(x = month, y = Mean_Ride_Time, color = rideable_type, group = rideable_type)) +
  geom_line() +
  geom_point() +
...
Total_UserCount_Monthly <- cyclist_2022 %>%
...
# Saving it to system 
write.csv(Total_UserCount_Monthly,"Monthly-UserCount-cyclist2022.csv", row.names = FALSE)
...
## Save Daily Trend
write.csv(Daily_Trend,"Daily-Trend-cyclist2022.csv", row.names = FALSE)
...
Weekday_UserCount <- cyclist_2022 %>%
...
write.csv(Weekday_mean_Usercount, "Weekday_mean-cyclist2022.csv", row.names = FALSE)


# Step 9: Time Range Analysis
...
write.csv(Time_range,"TimeInterval-cyclist2022.csv", row.names = FALSE)

write.csv(cyclist_2022, "cyclist-2022-v2.csv", row.names = FALSE)
...
weather_2022 <- importNOAA(year = 2022, code = "725340-14819")

## View the Data
View(weather_2022)

## Save it on system 
write.csv(weather_2022, "chicago_2022.csv", row.names = FALSE)
...
chicago_weather <- weather_2022 %>% select(all_of(weather_cols))

## Perform the left join on join_key
cyclist_2022 <- left_join(cyclist_2022, chicago_weather, by = "join_key")

## Remove the join_key column if it's not needed in the final result
cyclist_2022 <- cyclist_2022 %>% select(-join_key)
...
colSums(is.na(cyclist_2022))

## Filtering the dataset to see null values in temp 
filtered_air_temp <- subset(cyclist_2022, is.na(air_temp))

View(filtered_air_temp)

## Grouping the dataset to actually see for what date or time we don't have air_temp
...
cyclist_2022 <- subset(cyclist_2022, !is.na(air_temp))


# Ready to do EDA on weather parameters
hist(cyclist_2022$air_temp, main="Temperature Distribution", xlab="Temperature (°C)", col="lightblue", breaks=30)

hist(cyclist_2022$RH, main="RH Distribution", xlab="Relative Humidity", col="lightblue", breaks=30)

hist(cyclist_2022$ws, main="Wind Speed Distribution", xlab="ws (m/s)", col="lightblue", breaks=30)

## Checking the null values for windspeed column (didn't run, since it has been already cleaned by air temp)
filtered_w <- subset(cyclist_2022, is.na(ws))

filtered_ws <- filtered_w %>% 
  group_by(Date, Hour, month, ws) %>%
  summarise(count = n(), .groups = "drop")
View(filtered_ws)  ## Since there's only an hour in the whole dataset where we don't have wind-speed data, we can neglect it

colSums(is.na(cyclist_2022))

cyclist_Weather2022 <- cyclist_2022

write.csv(cyclist_Weather2022, "cyclist-2022(weather)-v3.csv", row.names = FALSE)

rm(cyclist_2022, chicago_weather, filtered_air_temp, weather_2022)


# Step 13: Starting With weather analytics 

## Extracting the year from Started_at column 
cyclist_Weather2022$year <- year(cyclist_Weather2022$started_at)

## Extracting the data that could be the part of EDA 
weather <- cyclist_Weather2022 %>%
...
weather <- weather %>%
  ...
  arrange(month)
write.csv(weather, "cyclist-2022-weather.csv", row.names = FALSE)

## Trying to Visualise how the user count differs with differing temp
...
colSums(is.na(cyclist_Weather2022))
